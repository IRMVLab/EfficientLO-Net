...
def PreProcess(PC_f1, PC_f2, T_gt, T_trans, T_trans_inv, aug_frame):    ####    pre process procedure
    ...
    for i in range(batch_size):
        ...
        # print('cur_T_trans', cur_T_trans)
        # print('cur_PC_f2_concat', cur_PC_f2_concat)
...
if __name__=='__main__':
    ...
    # file_path1 = "/tmp/data_odometry_velodyne/dataset/" + seq + "/velodyne/" + str(frame).zfill(6) + ".bin"
    # file_path2 = "/tmp/data_odometry_velodyne/dataset/" + seq + "/velodyne/" + str(frame+1).zfill(6) + ".bin"
    ...
    for i in range(100):
        ...
        print(i)
        # aug_frame = np.random.choice([1], size = batch_size, replace = True) # random choose aug frame 1 or 2
        ...
        # mask = torch.any(pos1 != 0, dim = -1)

        # pc1_soft = softmax_valid(pos1, pos1, mask_valid=mask)

    print('proj_total_time: ', time.time() - start)

    # out_proj = out_proj.cpu()
    # out_proj = out_proj.numpy()

    # np.savetxt('test_pytorch_project.txt', out_proj.reshape(-1, 3))