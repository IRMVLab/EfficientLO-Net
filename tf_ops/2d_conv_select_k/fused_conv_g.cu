...
__global__ void fused_conv_select_k_gpu(int batch_size, int H, int W, int npoints, int kernel_size_H, int kernel_size_W, int K, int flag_copy, float distance, int stride_h, int stride_w, const float *xyz1, const float *xyz2, const int *idx_n2, const int *random_hw, int *selected_bhw_idx, float *valid_idx, float *valid_in_dis_idx, float *selected_mask, int small_h, int small_w)
{
	...
	{		
		...
		int m_idx = 0;   //  m th  point in each kernel  
		...
		for (int s_idx = 0; s_idx < K; ++s_idx)  // knn  
		{
			...
			if ((flag_copy == 1) && (s_idx == 0)) // copy the first selected point in xyz2 for K times
			{
				...
				{	
					...
					selected_bhw_idx[current_n * K * 3 + k_idx * 3 + 1 ] = idx_h[s_idx];
					selected_bhw_idx[current_n * K * 3 + k_idx * 3 + 2 ] = idx_w[s_idx];
			...
			{	
				...
				selected_bhw_idx[current_n * K * 3 + s_idx * 3 + 0 ] = batch_index;
				selected_bhw_idx[current_n * K * 3 + s_idx * 3 + 1 ] = idx_h[s_idx];
				selected_bhw_idx[current_n * K * 3 + s_idx * 3 + 2 ] = idx_w[s_idx];
...
{
	fused_conv_select_k_gpu<<<batch_size,256>>>(batch_size, H, W, npoints, kernel_size_H, kernel_size_W, K, flag_copy, distance, stride_h, stride_w, xyz1, xyz2, idx_n2, random_hw, selected_bhw_idx, valid_idx, valid_in_dis_idx, selected_mask, small_h, small_w);
