...
public:
	...
	void Compute(OpKernelContext* context) override {
		...
		OP_REQUIRES(context, xyz1_tensor.dims() == 4 && xyz1_tensor.shape().dim_size(3) == 3, errors::InvalidArgument("FusedConvSelectK expects (batch_size, H, W, 3) xyz1 shape."));
		...
		OP_REQUIRES(context, xyz2_tensor.dims() == 4 && xyz2_tensor.shape().dim_size(1) == H2, errors::InvalidArgument("FusedConvSelectK expects (batch_size, H/stride_h, W/stride_w, 3) xyz2 shape."));
		...
		FusedConvSelectKLauncher(batch_size, H, W, npoints_, kernel_size_H_, kernel_size_W_, K_, flag_copy_, distance_, stride_h_, stride_w_, xyz1, xyz2, idx_n2, random_hw, selected_bhw_idx, valid_idx, valid_in_dis_idx, selected_mask, H2, W2);
	}
	private:
...
REGISTER_KERNEL_BUILDER(Name("FusedConvSelectK").Device(DEVICE_GPU), FusedConvSelectKGpuOp);
