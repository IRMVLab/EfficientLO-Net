...
void torch_FusedConvSelectKAddLauncher(
	...
	CHECK_INPUT(add_b_idx_tensor);
	CHECK_INPUT(add_h_idx_tensor);
	CHECK_INPUT(add_w_idx_tensor);
	...
	long *add_b_idx = add_b_idx_tensor.data<long>();
	long *add_h_idx = add_h_idx_tensor.data<long>();
	long *add_w_idx = add_w_idx_tensor.data<long>();
	...
	float *add_mask = add_mask_tensor.data<float>();
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("fused_conv_select_add_k",
          &torch_FusedConvSelectKAddLauncher,
          "torch_FusedConvSelectKAddLauncher kernel warpper");
}